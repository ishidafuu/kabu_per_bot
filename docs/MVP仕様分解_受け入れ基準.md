...
7. 通知時刻は `notify_timing` で切り替える（JST固定）
   ...
   - `OFF`: 通知しない
8. PER未定義日は、PER監視銘柄でも `【データ不明】` 通知を優先（自動PSR切替はしない）
9. データ取得フォールバック順は `四季報online → 株探 → Yahoo!ファイナンス`
10. 「来週」は月曜開始〜日曜終了の暦週で判定する
11. 通知チャネルは `Discord` のみを使用する
...
- Given: `notify_timing=IMMEDIATE` の銘柄がある
  When: 平日18:00の日次ジョブを実行する
  Then: `IMMEDIATE` 銘柄のみを通知評価する

- Given: `notify_timing=AT_21` の銘柄がある
...
## 6. P2確定仕様（2026-02-13反映）

以下は「未確定ポイント」から正式確定した仕様とする。

1. 通知時刻は `notify_timing`（`IMMEDIATE` / `AT_21`）で運用し、時刻基準はJST固定とする
2. PER未定義日は、PER監視銘柄でも `【データ不明】` 通知を優先する
3. データ取得フォールバック順は `四季報online → 株探 → Yahoo!ファイナンス` で固定する
4. 決算「来週」は月曜開始〜日曜終了の暦週で判定する
5. 通知チャネルは `Discord` のみを使用する

## 7. 補足方針（第2段階/技術選定）

1. SNS取得（第2段階）は `X API` を標準採用
2. AI要約（第2段階）は `Vertex AI Gemini` を標準採用
3. Grokは第2段階初期では採用しない（品質不足時のみ比較PoC）
4. Web管理画面は `React + TypeScript + Firebase Hosting` を採用
5. 管理APIは `FastAPI（Python）+ Cloud Run` を採用
