...
1. 市場データ本番取得（J-Quants※任意/株探/Yahoo）は実装済み。運用前に各データソース接続と欠損時ログ出力を確認する
2. BOTジョブは Cloud Run Jobs + Cloud Scheduler を前提に実装済み（IMMEDIATE帯、18:00、21:05、決算、IR/Grok、増分バックフィル）
3. Webは `/ops`（管理者）から全体設定確認・主要ジョブ手動実行・実行履歴確認が可能
...
1. `COOLDOWN_HOURS=2`（全体設定未登録時のデフォルト）
2. `INTEL_NOTIFICATION_MAX_AGE_DAYS=30`（IR/SNS通知対象期間のデフォルト）
3. `GROK_SNS_ENABLED=false`
4. `GROK_SNS_SCHEDULED_TIME=21:10`
5. `GROK_SNS_PER_TICKER_COOLDOWN_HOURS=24`

AI注目はウォッチリスト銘柄に対して常時試行されます（AI失敗時は速報通知のみ継続）。

AI要約の既定設定:

1. `VERTEX_AI_LOCATION=global`（必要に応じてリージョン変更）
2. `VERTEX_AI_MODEL=gemini-2.0-flash-001`
...
2. `errors=0` で終了する
3. 条件一致時のみ Discord通知送信が行われる（`sent=0` でも正常、`always_notify_enabled=true` 対象がない場合を含む）
4. 送信があった場合は Firestore `notification_log` に通知ログが保存される

21時向け銘柄（`notify_timing=AT_21`）の確認:
...
1. `IMMEDIATE` 銘柄は `--execution-mode daily`（既定）
2. `AT_21` 銘柄は `--execution-mode at_21`
3. 当日が非該当（割安条件未成立・欠損なし）かつ `always_notify_enabled=true` の対象がない場合、通知0件は正常
