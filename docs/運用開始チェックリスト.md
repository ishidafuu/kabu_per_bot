...
1. 市場データ本番取得（四季報/株探/Yahoo）の実データ収集処理は未実装（現在はStub）
2. BOTの日次ジョブはローカルデモ実行スクリプト中心
3. 本番運用はまず「通知経路・画面・APIの安定化」を優先する
...
1. Firestoreマイグレーションが適用済み
2. APIヘルスチェックが成功
3. Web（モックモード）が問題なく操作可能
4. Web E2E（モックUI + API結合）が成功
5. Discord疎通テスト成功
6. テストスイート全件成功
7. `.env`, `web/.env.local` に秘匿情報漏れがない（git管理外）
