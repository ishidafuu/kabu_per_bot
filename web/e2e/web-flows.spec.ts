...
test('ウォッチリスト詳細で技術プロファイルを割り当てできる', async ({ page }) => {
  await loginWithMock(page, '/watchlist/6501%3ATSE/detail');

  await expect(page.getByRole('heading', { name: '技術プロファイル' })).toBeVisible();
  await page.getByLabel('技術プロファイル').selectOption('system_small_growth');
  await page.getByLabel('manual override を有効にする').check();
  await page.getByRole('button', { name: 'プロファイル設定を保存' }).click();

  await expect(page.getByText('技術プロファイル設定を更新しました。')).toBeVisible();
  await expect(page.getByLabel('技術プロファイル')).toHaveValue('system_small_growth');
});

test('技術プロファイル画面でデフォルト表示とカスタム作成ができる', async ({ page }) => {
  await loginWithMock(page, '/technical-profiles');

  await expect(page.getByRole('heading', { name: '技術プロファイル' })).toBeVisible();
  await expect(page.locator('.technical-profile-card strong', { hasText: '大型・主力' })).toBeVisible();
  await expect(page.locator('.technical-profile-card strong', { hasText: '小型成長' })).toBeVisible();

  await page.getByRole('button', { name: '新規カスタムを作成' }).click();
  await page.getByRole('textbox', { name: 'profile_key', exact: true }).fill('swing_focus');
  await page.getByLabel('名前').fill('スイング重点');
  await page.getByLabel('説明').fill('短中期の値動きを重視するカスタム');
  await page.getByLabel('thresholds(JSON)').fill('{\n  "volume_spike": 2.4\n}');
  await page.getByLabel('weights(JSON)').fill('{\n  "trend": 30,\n  "demand": 35\n}');
  await page.getByRole('button', { name: '作成する' }).click();

  await expect(page.getByText('カスタムプロファイルを作成しました: スイング重点')).toBeVisible();
  await expect(page.locator('.technical-profile-card strong', { hasText: 'スイング重点' })).toBeVisible();
});
