...
main() {
  ...
  if [ -n "${JQUANTS_API_KEY_VALUE}" ]; then
    local backfill_env="${daily_env},JQUANTS_API_KEY=${JQUANTS_API_KEY_VALUE}"
    upsert_run_job "${BACKFILL_INCREMENTAL_JOB_NAME}" "scripts/run_incremental_backfill_job.py" "${backfill_env}" "${runtime_sa_email}"
  else
    log_warn "JQUANTS_API_KEY が未設定のため、増分バックフィルJob/Schedulerは作成しません。"
  fi
