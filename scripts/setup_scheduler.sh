...
main() {
  ...
  upsert_run_job "${INTELLIGENCE_JOB_NAME}" "scripts/run_intelligence_job.py,--intel-source,ir_only" "${intelligence_env}" "${runtime_sa_email}"
  if [ "${GROK_SECRET_AVAILABLE}" = true ]; then
    upsert_run_job "${GROK_JOB_NAME}" "scripts/run_intelligence_job.py,--intel-source,grok_only,--respect-grok-schedule" "${intelligence_env}" "${runtime_sa_email}" "DISCORD_WEBHOOK_URL=${SECRET_NAME}:latest,GROK_API_KEY=${GROK_SECRET_NAME}:latest"
  else
