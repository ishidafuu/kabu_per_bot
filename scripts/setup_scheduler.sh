...
INTELLIGENCE_CRON="${INTELLIGENCE_CRON:-*/1 * * * *}"
...
主なオプション:
  ...
  --intelligence-cron "<cron>" intelligenceジョブのcron（既定: */1 * * * *）
...
補足:
  ...
  - GROK_API_KEY が環境変数にある場合、intelligenceジョブ用 Secret に登録します。
EOF
}
...
ensure_grok_secret() {
  if gcloud secrets describe "${GROK_SECRET_NAME}" --project "${PROJECT_ID}" >/dev/null 2>&1; then
    ...
    log_warn "GROK_API_KEY 未設定かつ Secret 未作成のため、intelligenceジョブはIRのみで動作します。"
    GROK_SECRET_AVAILABLE=false
    return
  fi
  ...
  version_count="$(gcloud secrets versions list "${GROK_SECRET_NAME}" \
    ...
    --format='value(name)' 2>/dev/null | wc -l | tr -d ' ')"
  if [ "${version_count}" = "0" ]; then
    log_warn "Secret ${GROK_SECRET_NAME} にバージョンがないため、intelligenceジョブはIRのみで動作します。"
    GROK_SECRET_AVAILABLE=false
    return
  fi
...
main() {
  ...
  if [ "${GROK_SECRET_AVAILABLE}" = true ]; then
    upsert_run_job "${INTELLIGENCE_JOB_NAME}" "scripts/run_intelligence_job.py,--respect-grok-schedule" "${intelligence_env}" "${runtime_sa_email}" "DISCORD_WEBHOOK_URL=${SECRET_NAME}:latest,GROK_API_KEY=${GROK_SECRET_NAME}:latest"
  else
    upsert_run_job "${INTELLIGENCE_JOB_NAME}" "scripts/run_intelligence_job.py,--respect-grok-schedule" "${intelligence_env}" "${runtime_sa_email}"
  fi
